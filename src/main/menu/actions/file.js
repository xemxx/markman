...
const handleResponseForExport = async (
  e,
  { type, content, pathname, title, pageOptions }
) => {
...
const handleResponseForSave = async (
  e,
  { id, filename, markdown, pathname, options, defaultPath }
) => {
  const win = BrowserWindow.fromWebContents(e.sender)
  ...
  if (!filePath) {
    const { filePath: dialogPath, canceled } = await dialog.showSaveDialog(
      win,
      {
        defaultPath: path.join(
          defaultPath || getPath('documents'),
          `${recommendFilename}.md`
        )
      }
    )
...
const showUnsavedFilesMessage = async (win, files) => {
  const { response } = await dialog.showMessageBox(win, {
    ...
    defaultId: 0,
    message: `Do you want to save the changes you made to ${files.length} ${
...
ipcMain.on('mt::save-tabs', (e, unsavedFiles) => {
  Promise.all(unsavedFiles.map(file => handleResponseForSave(e, file))).catch(
    log.error
  )
})
...
ipcMain.on(
  ...
  async (e, { id, filename, markdown, pathname, options, defaultPath }) => {
    const win = BrowserWindow.fromWebContents(e.sender)
    let recommendFilename = getRecommendTitleFromMarkdownString(markdown)
    if (!recommendFilename) {
      recommendFilename = filename || 'Untitled'
...
ipcMain.on('mt::rename', async (e, { id, pathname, newPathname }) => {
  ...
  const doRename = () => {
    fs.rename(pathname, newPathname, err => {
      if (err) {
        log.error(
          `mt::rename: Cannot rename "${pathname}" to "${newPathname}".\n${err.stack}`
        )
        return
      }
  ...
  if (!isFile(newPathname)) {
    ...
    const { response } = await dialog.showMessageBox(win, {
      ...
      defaultId: 1,
      message: `The file "${path.basename(
        newPathname
...
ipcMain.on('mt::response-file-move-to', async (e, { id, pathname }) => {
  ...
  if (filePath && !canceled) {
    fs.rename(pathname, filePath, err => {
      if (err) {
        log.error(
          `mt::rename: Cannot rename "${pathname}" to "${filePath}".\n${err.stack}`
        )
        return
      }
...
ipcMain.on('mt::format-link-click', (e, { data, dirname }) => {
  ...
  if (
    !path.isAbsolute(data.href) &&
    isMarkdownFile(path.join(dirname, data.href))
  ) {
